...
def objfunc_time_series(par, beta, F, K, time, MKT):
    """
    Sum of Square Residuals (SSR) function for the SABR model.
    It is used as an input in the calibration phase.
    The goal of calibration is to minimize the output of this function.
    ...
    for j in range(len(K)):
        if MKT[j] == 0: #we interpret 0 as missing data - very unlikely that market volatility is 0!!
            ...
            V = (F*K[j])**((1-beta)/2)
            B = 1 + (((1-beta)**2 * alpha**2)/(24*(V**2)) + (rho*beta*alpha*nu)/(4*V) + ((2-3*(rho**2))*(nu**2))/(24)) * time[j]
            VOL = (alpha/V)*B
            diff = VOL - MKT[j]
        elif F != K[j]: #NON-ATM formula
            assert False
...
class SABR_swaption():
    ...
    def __init__(self, F, K, time, vols, beta=0.5,
                 ...
                 starting_params=[0.001, 0, 0.001], time_series=False):
    ...
    def calibration(self, calibration, starting_params):
        ...
        if calibration == "SLS_SciPy":
            ...
            if self.time_series:
                res = minimize(objfunc_time_series, x0=starting_params,
        ...
        # Calibration using SciPy's DifferentialEvolution
            ...
            if self.time_series:
                assert False, "Time series not implemented for DE_SciPy"
    ...
    def predict(self, K=None):
        ...
        """
        if K:
            return [SABR(self.alpha, self.beta, self.rho, self.nu, self.F, k, self.time) for k in (K + self.shift)]
        else:
